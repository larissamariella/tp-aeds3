...
public class LerCSV {
    ...
    Livro [] lerArquivoCSV() throws IOException {
        String nomeArquivo = "arq2.csv";
        RandomAccessFile arq = new RandomAccessFile(nomeArquivo, "rw");
        RandomAccessFile arqByte = new RandomAccessFile("arq2.db", "rw");
    ...
    Livro lerStringLivro(String str, Livro livro, int ultimoID) {
        // A variável indice controla a posição da linha lida que sera utilizada para
        // cada trecho dos atributos
        int indice = 0;
    ...
    // Separa a string do autor, retornando a posição da string que acaba o atributo
    // autor
    ...
    int lerData(String str, Livro livro, int i) {
        ...
        if (!data.equals(""))
            livro.setData(Util.formatarData(data));
        return posicao;
    }
